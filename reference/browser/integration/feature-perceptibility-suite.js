export const featurePerceptibilitySuite = Object.freeze({
  ...
  async run({ app, helpers:{ assert } }) {
    ...
    try {
      ...
      const optionsButton=document.querySelector('#livingJournalOptionsOpen'); assert(optionsButton,'FPD-02 Journal options gateway unavailable'); optionsButton.click(); await new Promise((resolve)=>setTimeout(resolve,10));
      const archiveTarget=app.guide.evaluateActionTarget('element.living-journal.archive.open'); assert(archiveTarget.state==='AVAILABLE','FPD-02 permission.none archive target not admitted through Journal options'); document.querySelector('#livingJournalOptionsClose')?.click(); await new Promise((resolve)=>setTimeout(resolve,10)); checks.push('FPD-02 permission.none Archive admitted through Journal options');
