...
export async function runLivingJournalProof({app,helpers:{delay,assert},viewportClass=null}){
  ...
  const prev=document.querySelector('#livingJournalPrevious'),next=document.querySelector('#livingJournalNext');for(const target of [prev,next])assert(target?.getBoundingClientRect().height>=44,'J04 explicit page control below 44px');
  ...
  const journeyBeforeNote=app.navigation.fullJourney(),identityBeforeNote=controller.canonicalThenIdentity();const input=document.querySelector('#livingJournalMarginaliaInput');input.value='temporary proof note';document.querySelector('#livingJournalMarginaliaAdd').click();await delay(10);assert(controller.snapshot().marginaliaCount===1&&controller.canonicalThenIdentity()===identityBeforeNote&&JSON.stringify(app.navigation.fullJourney())===JSON.stringify(journeyBeforeNote),'J11 local marginalia crossed semantic/source boundary');checks.push('J11');
