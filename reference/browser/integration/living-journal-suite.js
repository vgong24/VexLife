...
export async function runLivingJournalProof({app,helpers:{delay,assert},viewportClass=null}){
  ...
  await app.openLivingJournal({loadMemory:false});await delay(30);
  ...
  app.returnToTerrain();await delay(10);await app.openLivingJournal({loadMemory:false});await delay(10);snap=controller.snapshot();assert(snap.totalMarginaliaCount===0&&snap.truthClass==='CURRENT_SYNTHETIC_REFERENCE'&&localStorage.getItem('vexlife.living-journal')===null,'J12 close/reopen made marginalia look durable/canonical');checks.push('J12');
  ...
  let effectfulRejected=false,incompleteEffectsRejected=false,mismatchedConversationRejected=false,mismatchedSourceRefsRejected=false;
  try{controller.setData({...memoryFixture,effects:{...allFalseEffects,memoryMutated:true}});}catch{effectfulRejected=true;}
  try{controller.setData({...memoryFixture,effects:{}});}catch{incompleteEffectsRejected=true;}
  try{controller.setData({...memoryFixture,pages:[{...memoryPage,sourceDescent:{...memoryPage.sourceDescent,observedCurrentConversationHeadSha256:'b'.repeat(64)}}]});}catch{mismatchedConversationRejected=true;}
  try{controller.setData({...memoryFixture,pages:[{...memoryPage,sourceDescent:{...memoryPage.sourceDescent,observedCommittedSourceEventRefs:[...memoryPage.sourceDescent.observedCommittedSourceEventRefs].reverse()}}]});}catch{mismatchedSourceRefsRejected=true;}
  assert(effectfulRejected&&incompleteEffectsRejected&&mismatchedConversationRejected&&mismatchedSourceRefsRejected,'J16 malformed/effectful Memory packet was admitted');
  controller.setData(memoryFixture);await app.openLivingJournal({loadMemory:false});await delay(15);snap=controller.snapshot();
  ...
  assert(snap.currentDailyStratumRef===memoryPage.currentDailyStratumRef&&snap.currentDailyStratumSha256===memoryPage.currentDailyStratumSha256&&snap.dayRef===memoryPage.dayRef&&snap.dayIndex===memoryPage.dayIndex&&snap.sourceConversationHeadSha256===memoryPage.sourceConversationHeadSha256&&snap.sourceScoreHeadSha256===memoryPage.sourceScoreHeadSha256&&snap.sourceSemanticAuthorityHeadSha256===memoryPage.sourceSemanticAuthorityHeadSha256,'J16 snapshot lost Memory currentness/source identity');
  ...
  const memoryArticle=document.querySelector('.living-journal-page[data-current="true"]');assert(memoryArticle?.dataset.currentDailyStratumRef===memoryPage.currentDailyStratumRef&&memoryArticle?.dataset.currentDailyStratumSha256===memoryPage.currentDailyStratumSha256&&memoryArticle?.dataset.dayRef===memoryPage.dayRef&&Number(memoryArticle?.dataset.dayIndex)===memoryPage.dayIndex&&memoryArticle?.dataset.sourceConversationHeadSha256===memoryPage.sourceConversationHeadSha256&&memoryArticle?.dataset.sourceScoreHeadSha256===memoryPage.sourceScoreHeadSha256&&memoryArticle?.dataset.sourceSemanticAuthorityHeadSha256===memoryPage.sourceSemanticAuthorityHeadSha256,'J16 Memory DOM currentness/source bindings are not exact');
  ...
  document.querySelector('#livingJournalSource').click();await delay(5);snap=controller.snapshot();const memorySourceStatus=document.querySelector('#livingJournalSourceStatus')?.textContent??'',sourcePacket=snap.lastSourcePacket;assert(snap.sourceDoorRef===memoryPage.pageRef&&JSON.stringify(snap.sourceDoorRefs)===JSON.stringify(memoryPage.sourceBindings.map((binding)=>binding.eventRef))&&memorySourceStatus.includes(memoryPage.sourceBindings[0].eventRef)&&!memorySourceStatus.includes(memoryPage.summary)&&sourcePacket?.statementRef===memoryPage.statementRef&&sourcePacket?.pageRef===memoryPage.pageRef&&sourcePacket?.currentDailyStratumRef===memoryPage.currentDailyStratumRef&&sourcePacket?.currentDailyStratumSha256===memoryPage.currentDailyStratumSha256&&sourcePacket?.dayRef===memoryPage.dayRef&&sourcePacket?.dayIndex===memoryPage.dayIndex&&sourcePacket?.sourceConversationHeadSha256===memoryPage.sourceConversationHeadSha256&&sourcePacket?.sourceScoreHeadSha256===memoryPage.sourceScoreHeadSha256&&sourcePacket?.sourceSemanticAuthorityHeadSha256===memoryPage.sourceSemanticAuthorityHeadSha256&&sourcePacket?.sourceDescent?.rawSourceContentIncluded===false&&!Object.hasOwn(sourcePacket??{},'eventRef'),'J16 Memory source door did not preserve exact currentness/source packet without raw/event impersonation');
  document.querySelector('#livingJournalRevisit').click();await delay(10);snap=controller.snapshot();const revisitPacket=snap.lastRevisitPacket;assert(app.navigation.semanticFrame().screenRef==='screen.vexlife.terrain'&&snap.eventRef===null&&revisitPacket?.statementRef===memoryPage.statementRef&&revisitPacket?.pageRef===memoryPage.pageRef&&revisitPacket?.currentDailyStratumRef===memoryPage.currentDailyStratumRef&&revisitPacket?.currentDailyStratumSha256===memoryPage.currentDailyStratumSha256&&revisitPacket?.dayRef===memoryPage.dayRef&&revisitPacket?.dayIndex===memoryPage.dayIndex&&revisitPacket?.sourceConversationHeadSha256===memoryPage.sourceConversationHeadSha256&&revisitPacket?.sourceScoreHeadSha256===memoryPage.sourceScoreHeadSha256&&revisitPacket?.sourceSemanticAuthorityHeadSha256===memoryPage.sourceSemanticAuthorityHeadSha256&&!Object.hasOwn(revisitPacket??{},'eventRef'),'J16 Memory revisit invented event identity or lost page/current-source identity');
  const emptyMemory=Object.freeze({...memoryFixture,realJournalBodyLoaded:false,pageCount:0,pages:Object.freeze([])});controller.setData(emptyMemory);await app.openLivingJournal({loadMemory:false});await delay(10);snap=controller.snapshot();assert(snap.pageCount===0&&snap.pageRef===null&&snap.visiblePageCount===0&&document.querySelectorAll('#livingJournalSpread .living-journal-page').length===0&&document.querySelector('#livingJournalPrevious')?.disabled&&document.querySelector('#livingJournalNext')?.disabled,'J16 zero-page CURRENT Memory projection is not safe');
