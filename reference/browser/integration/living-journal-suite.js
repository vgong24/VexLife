...
export async function runLivingJournalProof({app,helpers:{delay,assert},viewportClass=null}){
  ...
  const memoryArticle=document.querySelector('.living-journal-page[data-current="true"]');assert(memoryArticle?.dataset.currentDailyStratumRef===memoryPage.currentDailyStratumRef&&memoryArticle?.dataset.sourceConversationHeadSha256===memoryPage.sourceConversationHeadSha256&&memoryArticle?.dataset.sourceScoreHeadSha256===memoryPage.sourceScoreHeadSha256&&memoryArticle?.dataset.sourceSemanticAuthorityHeadSha256===memoryPage.sourceSemanticAuthorityHeadSha256,'J16 Memory DOM currentness/source bindings are not exact');
  ...
  document.querySelector('#livingJournalSource').click();await delay(5);snap=controller.snapshot();const memorySourceStatus=document.querySelector('#livingJournalSourceStatus')?.textContent??'',sourcePacket=snap.lastSourcePacket;assert(snap.sourceDoorRef===memoryPage.pageRef&&JSON.stringify(snap.sourceDoorRefs)===JSON.stringify(memoryPage.sourceBindings.map((binding)=>binding.eventRef))&&memorySourceStatus.includes(memoryPage.sourceBindings[0].eventRef)&&!memorySourceStatus.includes(memoryPage.summary)&&sourcePacket?.statementRef===memoryPage.statementRef&&sourcePacket?.pageRef===memoryPage.pageRef&&sourcePacket?.sourceConversationHeadSha256===memoryPage.sourceConversationHeadSha256&&sourcePacket?.sourceDescent?.rawSourceContentIncluded===false&&!Object.hasOwn(sourcePacket??{},'eventRef'),'J16 Memory source door did not preserve exact source packet without raw/event impersonation');
  document.querySelector('#livingJournalRevisit').click();await delay(10);snap=controller.snapshot();const revisitPacket=snap.lastRevisitPacket;assert(app.navigation.semanticFrame().screenRef==='screen.vexlife.terrain'&&snap.eventRef===null&&revisitPacket?.statementRef===memoryPage.statementRef&&revisitPacket?.pageRef===memoryPage.pageRef&&revisitPacket?.sourceConversationHeadSha256===memoryPage.sourceConversationHeadSha256&&!Object.hasOwn(revisitPacket??{},'eventRef'),'J16 Memory revisit invented event identity or lost statement/page/current-source identity');
