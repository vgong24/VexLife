...
export function createGuideController({ state, t, navigation, elementByRef, chat }) {
  ...
  function setOpen(open, { focus = false } = {}) { state.guideOpen = Boolean(open); windowElement.hidden = !state.guideOpen; localStorage.setItem('vexlife.guide.open', String(state.guideOpen)); if (state.guideOpen) { windowElement.classList.toggle('is-minimized', state.guideMinimized === true); updateFrame(); if (avoidDeclaredControls()) persistGeometry(); if (focus && !state.guideMinimized) $('#guideInput')?.focus(); } }
  function summon() { navigation.navigate('element.vex.summon', {}, 'action.vex.summon'); state.guideMinimized = false; localStorage.setItem('vexlife.guide.minimized', 'false'); windowElement.classList.remove('is-minimized'); setOpen(true, { focus: true }); }
