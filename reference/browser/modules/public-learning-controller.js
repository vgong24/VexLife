...
export function validatePublicLearningBrowserInputs({ projection, registry, catalogs }) {
  need(projection?.schemaVersion === PUBLIC_PROJECTION_SCHEMA, 'B3 requires a public-learning projection');
...
export function buildPublicPresentationTerrain(projection) {
  ...
  while (queue.length) {
    ...
    for (const childRef of children) {
      ...
      need(child, `forward B1 presentation edge targets an unavailable ref: ${childRef}`);
...
export function createPublicLearningController({ projection, registry, catalogs, root = document }) {
  ...
  need(terrainByRef.has(initialRef), `initial public ref is outside the B3 presentation tree: ${initialRef}`);
  ...
  function renderRelationshipList(container, node, leaf) {
    ...
    for (const relation of relationRows(node, leaf)) {
      ...
      item.innerHTML = `<strong>${escapeHtml(label)}</strong><small>${escapeHtml(relation.type)} · ${escapeHtml(relation.ref)}</small>`;
