...
function assertArchiveData(data){
  ...
  if(data.selectedDay!==null){const selected=data.selectedDay;if(!object(selected)||selected.temporalTruthClass!==ARCHIVE_DAY_TRUTH||selected.currentNowEvaluated!==false||selected.rawConversationContentIncluded!==false||!Array.isArray(selected.pages)||!object(selected.sourceDescent)||selected.sourceDescent.rawConversationContentIncluded!==false)throw new Error('Living Journal selected historical day boundary is invalid');selected.pages.forEach(assertArchivePage);}
  return data;
}
...
export function createLivingJournalController({state,data,t,navigation,onSourceOpen=()=>{},onRevisit=()=>{}}){
  ...
  function renderPage(index){
    ...
    ordinal.textContent=view.mode==='SYNTHETIC'?`${String(view.sequence).padStart(2,'0')} · ${view.eventRef}`:`${String(index+1).padStart(2,'0')} · ${view.statementRef}`;
    if(view.mode==='ARCHIVE'){
      const summary=document.createElement('section');summary.className='living-journal-time living-journal-memory-summary';summary.dataset.memoryState='HISTORICAL_COMMITTED';summary.dataset.temporalTruthClass=ARCHIVE_DAY_TRUTH;summary.dataset.currentNowEvaluated='false';
      ...
      article.append(ordinal,summary,source);return article;
    }
    if(view.mode==='MEMORY'){
