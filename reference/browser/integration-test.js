...
export async function runBrowserIntegration() {
  ...
  try {
    ...
    const current = app.terrain.currentRef(); const children = app.terrain.childRefs(current); if (children.length) { const beforeSequence=app.terrain.transitionSnapshot().sequence,motionBefore=app.terrain.liveGeometrySnapshot(),travelPromise=app.terrain.travel(children[0], 'in'); await delay(70); const exiting=app.terrain.transitionSnapshot(),world=document.querySelector('#terrainWorld'),motionDuring=app.terrain.liveGeometrySnapshot(),beforeTarget=motionBefore.nodes.find((node)=>node.ref===children[0]),duringTarget=motionDuring.nodes.find((node)=>node.ref===children[0]),beforePeer=motionBefore.nodes.find((node)=>node.ref!==children[0]),duringPeer=beforePeer?motionDuring.nodes.find((node)=>node.ref===beforePeer.ref):null; assert(exiting.phase==='EXITING'&&exiting.fromRef===current&&exiting.toRef===children[0]&&exiting.sequence===beforeSequence+1,'D09 semantic travel did not expose a bounded EXITING transition'); assert(world.dataset.transitionPhase==='EXITING'&&Number(getComputedStyle(world).opacity)<1&&Number(getComputedStyle(world).opacity)>.55,'D09 spatial handoff should be visible without collapsing into a near-black world fade'); assert(beforeTarget&&duringTarget&&radialDistance(duringTarget)<radialDistance(beforeTarget)-35,'D09 selected semantic object did not visibly travel toward current-context center before handoff'); assert(!beforePeer||!duringPeer||radialDistance(duringPeer)>radialDistance(beforePeer)+5,'D09 neighboring semantic objects did not yield space during selected-object promotion'); await travelPromise; const settled=app.terrain.transitionSnapshot(); assert(app.terrain.currentRef() === children[0], 'D09 semantic travel failed'); assert(settled.phase==='IDLE'&&settled.sequence===beforeSequence+1&&world.dataset.transitionPhase==='IDLE'&&Number(getComputedStyle(world).opacity)===1,'D09 semantic travel did not settle through ARRIVING to IDLE'); if(e28Poc01)e28Poc01.transition={fromRef:current,toRef:children[0],observedExit:exiting,settled}; const siblings = app.terrain.siblingRefs(); if (siblings.length > 1) { const beforeDepth = app.terrain.viewportProjection().semanticDepth; const moved = await app.terrain.navigateSibling('NEXT'); if (moved) assert(app.terrain.viewportProjection().semanticDepth === beforeDepth, 'D09 sibling travel changed hierarchy depth'); } }
