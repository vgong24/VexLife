...
import { canonicalize, requireSafeRelativePath, semanticHash, writeJson } from './utils.mjs';
import { buildAdmissionAdapterSourceHash, buildAdmissionLedgerPath, cleanupDisposableGitRepository, formBuildRecoveryReceipt, prepareDisposableGitRepository, safeRemoveDisposableRepository } from '../../scripts/build-admission-check.mjs';
const SHA1_PATTERN = /^[a-f0-9]{40}$/;
const SHA256_PATTERN = /^[a-f0-9]{64}$/;
const FORBIDDEN_GIT_ARGUMENTS = new Set([
 'push', 'fetch', 'pull', 'clone', 'rebase', 'reset', 'amend', '--amend', '--force', '-f',
 'filter-branch', 'filter-repo', 'update-ref', 'remote', 'branch-delete', '-D', '-d'
]);
const ALLOWED_GIT_VERBS = new Set([
 'init', 'config', 'add', 'commit', 'switch', 'checkout', 'branch', 'rev-parse', 'diff', 'status',
 'remote', 'show', 'log', 'hash-object'
]);
const MARKER_PATH = '.vexlife-disposable-repository.json';
...
function requireSha1(value, label) {
 if (!SHA1_PATTERN.test(value ?? '')) throw new Error(`${label} must be a lowercase Git SHA-1`);
 return value;
}
...
function assertTypedGitArguments(args) {
 if (!Array.isArray(args) || args.length === 0 || !ALLOWED_GIT_VERBS.has(args[0])) {
 throw new Error('unregistered Git command is not allowed');
 }
 for (const arg of args) {
 if (typeof arg !== 'string' || arg.includes('\0') || arg.includes('\n') || arg.includes('\r')) {
 throw new Error('Git command arguments must be bounded strings');
 }
 if (FORBIDDEN_GIT_ARGUMENTS.has(arg)) throw new Error(`forbidden Git operation: ${arg}`);
 if (/^(?:https?|ssh|git):/i.test(arg) || /^git@/i.test(arg)) {
 throw new Error('network or remote Git argument is forbidden');
 }
 }
}
function runGit(root, args, { label = args.join(' '), env = {}, allowFailure = false } = {}) {
...
function canonicalContentSha256(content) {
...
function validateExecutionBindings(request, admission, replacementContent, registry) {
 ...
 const requestCore = clone(request);
 delete requestCore.buildRequestRef;
 delete requestCore.semanticFingerprint;
 const requestFingerprint = semanticHash(requestCore);
 if (request.semanticFingerprint !== requestFingerprint ||
 request.buildRequestRef !== `request.build-admission.${requestFingerprint.slice(0, 24)}`) {
 throw new Error('build request is forged, re-addressed, or same-ref/different-meaning');
 }
 const admissionCore = clone(admission);
 delete admissionCore.buildAdmissionRef;
 delete admissionCore.semanticFingerprint;
 const admissionFingerprint = semanticHash(admissionCore);
 if (admission.semanticFingerprint !== admissionFingerprint ||
 admission.buildAdmissionRef !== `admission.build-admission.${admissionFingerprint.slice(0, 24)}`) {
 ...
 throw new Error('build admission is detached from the exact request');
 }
 if (admission.currentness !== 'CURRENT' || admission.externalEffectsAuthorized !== true) {
 throw new Error('build admission is not current or effect-authorizing');
 }
 if (canonicalContentSha256(replacementContent) !== request.replacementContentSha256) {
 throw new Error('replacement content SHA-256 mismatch');
 }
 if (gitBlobSha(replacementContent) !== request.expectedAfterBlobSha) {
 throw new Error('replacement content Git blob mismatch');
 }
 if (request.mutationPath !== registry.adapter.fixturePath) {
 throw new Error('request mutation path is not the registered adapter fixture path');
 }
}
export function executeDisposableLocalGitEffect({
 ...
 if (prior.buildRequestRef !== request.buildRequestRef || prior.buildRequestFingerprint !== request.semanticFingerprint ||
 prior.buildAdmissionRef !== admission.buildAdmissionRef || prior.buildAdmissionFingerprint !== admission.semanticFingerprint) {
 throw new Error('build request fingerprint ledger conflicts with prior meaning');
 }
 const current = readCommitEvidence(repositoryPath, safeMutationPath);
 if (current.commitSha !== prior.commitSha || current.commitTreeSha !== prior.commitTreeSha ||
 current.afterBlobSha !== prior.afterBlobSha || current.diffFingerprint !== prior.diffFingerprint) {
 ...
 const dirty = runGit(repositoryPath, ['status', '--porcelain=v1', '--untracked-files=all'], { label: 'Git pre-commit status' }).stdout
 .split(/\r?\n/).filter(Boolean);
 const expectedDirty = ` M ${safeMutationPath}`;
 if (dirty.length !== 1 || dirty[0] !== expectedDirty) {
 throw new Error(`unexpected pre-commit worktree inventory: ${dirty.join(', ')}`);
 }
 runGit(repositoryPath, ['switch', '-c', effectBranch], { label: 'Git effect branch creation' });
 runGit(repositoryPath, ['add', '--', safeMutationPath], { label: 'Git exact effect path add' });
 const stagedPaths = runGit(repositoryPath, ['diff', '--cached', '--name-only', '--'], { label: 'Git staged path inventory' }).stdout
 .split(/\r?\n/).filter(Boolean).sort();
 if (stagedPaths.length !== 1 || stagedPaths[0] !== safeMutationPath) {
 throw new Error(`unexpected staged path inventory: ${stagedPaths.join(', ')}`);
 }
 const remaining = runGit(repositoryPath, ['status', '--porcelain=v1', '--untracked-files=all'], { label: 'Git exact pre-commit status' }).stdout
 .split(/\r?\n/).filter(Boolean);
 if (remaining.length !== 1 || remaining[0] !== `M  ${safeMutationPath}`) {
 throw new Error(`unexpected staged/unstaged inventory: ${remaining.join(', ')}`);
 }
 if (failurePhase === 'COMMIT') throw new Error('injected COMMIT failure');
 runGit(repositoryPath, ['commit', '-m', request.commitMessage, '-m', `Signed-off-by: ${ADAPTER_AUTHOR_NAME} <${ADAPTER_AUTHOR_EMAIL}>`], {
 label: 'Git ordinary effect commit',
 env: { GIT_AUTHOR_DATE: completedAt, GIT_COMMITTER_DATE: completedAt }
 });
 ...
 if (readback.changedPaths.length !== 1 || readback.changedPaths[0] !== safeMutationPath) {
 throw new Error('effect readback changed path inventory mismatch');
 }
 const core = {
 ...
 const effectReceipt = deepFreeze({
 ...core,
 buildEffectReceiptRef: `effect.build-admission.local-git.${semanticFingerprint.slice(0, 24)}`,
 semanticFingerprint
 });
 ...
 rollbackSucceeded = restored.headSha === request.expectedHeadSha && restored.treeSha === request.expectedTreeSha &&
 restored.mutationBlobSha === request.expectedBeforeBlobSha && restored.workingTree === 'CLEAN';
 ...
 request,
 admission,
 repositoryEvidence: baselineEvidence,
 error,
 formedAt,
 completedAt,
 rollbackAttempted,
...
export function validateBuildEffectReceipt(effectReceipt, { request, admission, workspaceRoot, repositoryPath, registry }) {
 ...
 const expectedRef = `effect.build-admission.local-git.${expectedFingerprint.slice(0, 24)}`;
 if (effectReceipt.semanticFingerprint !== expectedFingerprint || effectReceipt.buildEffectReceiptRef !== expectedRef) {
 throw new Error('build effect receipt is caller-authored, forged, or re-addressed');
 }
 if (effectReceipt.contractRef !== registry.effectReceiptContract.contractRef ||
 effectReceipt.schemaVersion !== registry.effectReceiptContract.schemaVersion ||
 effectReceipt.adapterRef !== registry.adapter.adapterRef ||
 effectReceipt.adapterSourceRef !== registry.adapter.sourceRef ||
 effectReceipt.adapterSourceHash !== buildAdmissionAdapterSourceHash(registry) ||
 effectReceipt.formationRef !== registry.adapter.formationRef ||
 effectReceipt.effectScope !== registry.adapter.effectScope ||
 effectReceipt.externalEffectsExecuted !== true || effectReceipt.networkUsed !== false ||
 effectReceipt.remoteConfigured !== false) {
 throw new Error('build effect receipt does not bind the registered real local adapter');
 }
 if (effectReceipt.buildRequestRef !== request.buildRequestRef ||
 effectReceipt.buildRequestFingerprint !== request.semanticFingerprint ||
 effectReceipt.buildAdmissionRef !== admission.buildAdmissionRef ||
 effectReceipt.buildAdmissionFingerprint !== admission.semanticFingerprint) {
 ...
 if (semanticHash(current.changedPaths) !== semanticHash(effectReceipt.changedPaths) ||
 effectReceipt.changedPaths.length !== 1 || effectReceipt.changedPaths[0] !== request.mutationPath) {
 throw new Error('build effect direct changed-path readback mismatch');
 }
 const resolved = resolveDisposableRepositoryPath(workspaceRoot, repositoryPath);
 const pathHash = crypto.createHash('sha256').update(resolved.canonicalRepository).digest('hex');
 if (pathHash !== effectReceipt.repositoryCanonicalPathHash) throw new Error('build effect repository path binding mismatch');
