...
const loadJson = (root, relativePath) => readJson(path.join(root, relativePath));
...
export function loadPurposeWorkspaceRegistry(root = VEXLIFE_ROOT) {
  ...
  return {
    registry,
    processPatterns: loadJson(root, registry.includes.processPatterns),
    completionContracts: loadJson(root, registry.includes.completionContracts),
    domainPacks: registry.includes.domainPacks.map((source) => loadJson(root, source)),
    root
  };
}
...
export function compileTaskStages(task, pattern) {
  ...
  if (arrays.some((items) => !Array.isArray(items) || items.length !== pattern.stagePurposes.length)) throw new Error(`${task.taskRef} stage binding length does not match ${pattern.processPatternRef}`);
...
export function validatePurposeWorkspaceRegistry(bundle) {
  ...
  const seen = { domain: new Set(), role: new Set(), task: new Set() };
  const addUnique = (kind, ref) => {
    if (!nonempty(ref) || seen[kind].has(ref)) errors.push(`duplicate or missing ${kind} ${ref ?? ''}`);
    seen[kind].add(ref);
  };
  for (const domain of domainPacks ?? []) {
    addUnique('domain', domain.domainRef);
    ...
    if (domain.primaryTaskRefs?.length !== 3) errors.push(`${domain.domainRef} must expose exactly three primary tasks`);
    const domainRoleRefs = new Set();
    for (const role of domain.roleLenses ?? []) {
      addUnique('role', role.roleLensRef); domainRoleRefs.add(role.roleLensRef);
  ...
  return {
    ok: errors.length === 0,
    errors,
    stats: { domains: seen.domain.size, roles: seen.role.size, tasks: seen.task.size, processPatterns: processPatterns?.length ?? 0, completionContracts: completionContracts?.length ?? 0 },
    semanticHash: semanticHash({ registry, processPatterns, completionContracts, domainPacks })
  };
}

function resolveTask(bundle, domainRef, taskRef = null) {
  ...
  const task = domain.tasks.find((item) => item.taskRef === selectedTaskRef);
  if (!task) throw new Error(`missing task ${selectedTaskRef}`);
  ...
  if (!pattern || !contract) throw new Error(`${selectedTaskRef} cannot resolve process/completion source`);
  return { domain, task, pattern, contract, stages: compileTaskStages(task, pattern) };
}
...
export function compilePurposeWorkspace({ root = VEXLIFE_ROOT, domainRef, taskRef = null, semanticDepth = 'DO' } = {}) {
  ...
  const resolved = resolveTask(bundle, domainRef, taskRef);
  const base = {
    featureRef: FEATURE_REF,
    workspaceRef: WORKSPACE_REF,
    plannedScreenRef: bundle.registry.workspaceDefinitions[0].screenRef,
    domainRef: resolved.domain.domainRef,
    domainLabel: resolved.domain.label,
    purposeCompletionQuestion: resolved.domain.purposeCompletionQuestion,
    semanticDepth,
    primaryTaskRefs: [...resolved.domain.primaryTaskRefs],
  ...
  if (semanticDepth === 'STEWARD') return { ...base, stages: resolved.stages, roleLenses: clone(resolved.domain.roleLenses), ledgerBindings: clone(resolved.domain.ledgerBindings ?? []), heldBoundaries: [...(resolved.domain.heldBoundaries ?? [])], entryContinuity: clone(bundle.registry.entryContinuityContract), sourcePlacement: clone(bundle.registry.sourcePlacement) };
  return base;
}
...
export function evaluatePurposeCompletion({ task, contract, stageReceipts = [], explicitBlockers = [] } = {}) {
  ...
  const evidenceCoverage = Object.fromEntries(contract.requiredEvidenceClasses.map((item) => [item, false]));
  for (const stage of task.compiledStages ?? []) {
    const receipt = receipts.get(stage.stageRef);
    if (receipt?.state === 'COMPLETE' || receipt?.state === 'HELD_EXTERNAL_EFFECT') if (stage.patternStagePurpose in stageCoverage) stageCoverage[stage.patternStagePurpose] = true;
    for (const item of receipt?.evidenceClasses ?? []) if (item in evidenceCoverage) evidenceCoverage[item] = true;
  }
  ...
  if (!stageReceipts.length) return { state: 'NOT_STARTED', stageCoverage, evidenceCoverage, blockers: [] };
...
export function simulateRoleRelay({ root = VEXLIFE_ROOT, scenario } = {}) {
  const bundle = loadPurposeWorkspaceRegistry(root);
  const resolved = resolveTask(bundle, scenario.domainRef, scenario.taskRef);
  const authority = new Set(scenario.authorityClassRefs ?? []);
  const stageReceipts = [];
  const relayEnvelopes = [];
  for (const [index, stage] of resolved.stages.entries()) {
    ...
    stageReceipts.push({ stageRef: stage.stageRef, state, evidenceClasses: authorized && !externalHeld ? [stage.requiredEvidenceClass] : [], blockers: !authorized ? [`missing ${stage.requiredAuthorityClassRef}`] : externalHeld ? ['external effect held'] : [] });
    relayEnvelopes.push({
      stageRef: stage.stageRef,
      speakerRoleRef: stage.ownerRoleRef,
      recipientRoleRefOrNull: scenario.recipientRoleRefOrNull ?? resolved.stages[index + 1]?.ownerRoleRef ?? null,
      communicationKind: stage.communicationKind,
      epistemicClass: scenario.epistemicClassByPurpose?.[stage.patternStagePurpose] ?? (stage.communicationKind === 'DIRECT_OBSERVATION' ? 'DIRECT_OBSERVATION' : stage.communicationKind === 'INTERPRETATION' ? 'PROFESSIONAL_INTERPRETATION' : stage.communicationKind === 'CORRECTION' ? 'HUMAN_REPORT' : 'VERIFIED_FACT'),
      relationshipClassRefs: [...(scenario.relationshipClassRefs ?? [])],
      requiredAuthorityClassRef: stage.requiredAuthorityClassRef,
      authoritySourcePresent: authorized,
      effectClass: stage.effectClass,
      effectPerformed: false
    });
  }
  const completion = evaluatePurposeCompletion({ task: { ...resolved.task, compiledStages: resolved.stages }, contract: resolved.contract, stageReceipts, explicitBlockers: scenario.explicitBlockers ?? [] });
  if (stageReceipts.some((item) => item.state === 'WAITING_FOR_AUTHORITY')) completion.state = 'WAITING_FOR_AUTHORITY';
  return { scenarioRef: scenario.scenarioRef, domainRef: resolved.domain.domainRef, taskRef: resolved.task.taskRef, completion, stageReceipts, relayEnvelopes, effects: { externalEffectPerformed: false, realPersonDataUsed: false } };
}
...
export function evaluateExperienceTestCase(testCase) {
  ...
  const state = ({ COMPLETE: 'COMPLETE_WITH_EVIDENCE', MISSING_AUTHORITY: 'WAITING_FOR_AUTHORITY', CONTRADICTED_SOURCE: 'CORRECTION_REQUIRED', PROTECTED_EFFECT: 'HELD', EXTERNAL_READY: 'READY_FOR_EXTERNAL_EFFECT', SUPERSESSION: 'SUPERSEDED' })[testCase.rule];
...
export function buildExperienceTopology({ root = VEXLIFE_ROOT, cases = [] } = {}) {
  ...
  addNode(bundle.registry.topologyContract.sourceVersionRef, 'SOURCE_VERSION');
  addNode(bundle.registry.entryContinuityContract.entryChainRef, 'ENTRY_CHAIN');
  addNode(bundle.registry.contextProgressionContract.anchorRef, 'CONTEXT_ANCHOR');
  addNode(bundle.registry.contextProgressionContract.evolutionRef, 'CONTEXT_EVOLUTION');
  addNode(bundle.registry.contextProgressionContract.continuationRef, 'CONTEXT_CONTINUATION');
  ...
  addEdge(bundle.registry.registryRef, bundle.registry.topologyContract.sourceVersionRef, 'SOURCE_VERSION');
  addEdge(bundle.registry.registryRef, bundle.registry.entryContinuityContract.entryChainRef, 'HAS_ENTRY_CHAIN');
  addEdge(bundle.registry.registryRef, bundle.registry.contextProgressionContract.anchorRef, 'HAS_CONTEXT_ANCHOR');
  addEdge(bundle.registry.contextProgressionContract.anchorRef, bundle.registry.contextProgressionContract.evolutionRef, 'EVOLVES_THROUGH');
  addEdge(bundle.registry.contextProgressionContract.evolutionRef, bundle.registry.contextProgressionContract.continuationRef, 'CONTINUES_AS');
  ...
  for (const item of bundle.registry.authorityClasses) { addNode(item.authorityClassRef, 'AUTHORITY_CLASS'); addEdge(bundle.registry.registryRef, item.authorityClassRef, 'DECLARES_AUTHORITY_CLASS'); }
  for (const item of bundle.registry.relationshipClasses) { addNode(item.relationshipClassRef, 'RELATIONSHIP_CLASS'); addEdge(bundle.registry.registryRef, item.relationshipClassRef, 'DECLARES_RELATIONSHIP_CLASS'); }
  for (const ref of bundle.registry.knownExternalRefs) addNode(ref, 'EXTERNAL_REF');
  const sourcePaths = ['blueprint/purpose-workspace-registry.json', bundle.registry.includes.processPatterns, bundle.registry.includes.completionContracts, ...bundle.registry.includes.domainPacks, 'src/core/purpose-workspace.mjs', 'fixtures/purpose-workspace/synthetic-scenarios.json', 'test/purpose-workspace.test.mjs', 'docs/PURPOSE-WORKSPACE-AND-EXPERIENCE-TOPOLOGY.md'];
  for (const sourcePath of sourcePaths) { const ref = `source-file.${sourcePath}`; addNode(ref, 'SOURCE_FILE', { path: sourcePath }); addEdge(bundle.registry.topologyContract.sourceVersionRef, ref, 'CONTAINS_SOURCE'); }
  for (const item of bundle.processPatterns) addNode(item.processPatternRef, 'PROCESS_PATTERN');
  for (const item of bundle.completionContracts) addNode(item.completionContractRef, 'COMPLETION_CONTRACT');
  ...
  const nodeRefs = nodes.map((item) => item.ref), known = new Set(nodeRefs);
  ...
  const missingRequiredNodeKinds = bundle.registry.topologyContract.requiredNodeKinds.filter((kind) => !presentKinds.has(kind));
  return {
    topologyRef: bundle.registry.topologyContract.topologyRef,
    sourceVersionRef: SOURCE_VERSION_REF,
    formedAt: bundle.registry.topologyContract.formedAt,
    nodes, edges,
    audit: { ok: !duplicateNodeRefs.length && !unresolvedEdges.length && !duplicateEdgeKeys.length && !humanVisibleOrphans.length && !missingRequiredNodeKinds.length, mapTracksItself: true, duplicateNodeRefs, unresolvedEdges, duplicateEdgeKeys, humanVisibleOrphans, missingRequiredNodeKinds },
