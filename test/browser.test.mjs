...
const moduleNames = fs.readdirSync(new URL('modules/', browserRoot)).filter((name)=>name.endsWith('.js')).sort(); const allJs=[app,...moduleNames.map((name)=>fs.readFileSync(new URL(`modules/${name}`,browserRoot),'utf8'))].join('\n');

test('static browser refs remain registry-addressable',()=>{const refs=[...html.matchAll(/data-node-ref="([^"]+)"/g)].map((m)=>m[1]);assert.ok(refs.length>=25);for(const ref of refs)assert.ok(registry.get(ref),`unregistered browser node ${ref}`)});
test('direct-root executable chain has no transitional Stage-B overlay',()=>{assert.match(html,/data-presentation-contract="contract\.vexlife\.e27\.authoritative-root\/v1"/);assert.match(html,/<script type="module" src="\.\/app\.js"><\/script>/);assert.match(app,/Direct-root browser requires accepted E2\.7 authoritative-root contract/);assert.doesNotMatch(app,/e27-terrain-convergence/);assert.doesNotMatch(css,/CANONICAL_STAGE_B_ROOT_STYLES/);assert.equal(fs.existsSync(new URL('e27-convergence.css',browserRoot)),false);assert.equal(fs.existsSync(new URL('modules/e27-terrain-convergence.js',browserRoot)),false)});
test('browser modules remain syntactically valid',()=>{for(const name of ['app.js',...moduleNames.map((name)=>`modules/${name}`)]){const result=spawnSync(process.execPath,['--check',fileURLToPath(new URL(name,browserRoot))],{encoding:'utf8'});assert.equal(result.status,0,`${name}: ${result.stderr}`)}});
test('current semantic owners remain present beneath the E2.7 body',()=>{for(const marker of ['createChatController','createNavigationController','createGuideController','createTerrainController','conversationKey','UNSENT_LOCAL_DRAFT','vexAvailability','vex.visible.name'])assert.match(allJs,new RegExp(marker.replaceAll('.','\\.')));assert.doesNotMatch(`${html}\n${allJs}`,/VexOrg Demo Company|Maya Chen/)});
