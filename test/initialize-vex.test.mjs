...
test('initialization plan exposes only the admitted product effects', () => {
  const plan = buildVexInitializationPlan({ profile, modelBundle, modelArtifacts: activeModelSelection.artifacts, home: 'C:/VexHome', homeState: 'FRESH_HOME_ALLOWED', hostEvidence: { platform: 'win32', architecture: 'x64' }, mode: 'candidate-qualification' });
...
test('ordinary initializer source consumes canonical resolver for models while runtime archives retain direct verified acquisition', () => {
  ...
  assert.match(source, /const modelArtifacts = modelSelection\.artifacts/u);
  assert.match(source, /for \(const artifact of modelArtifacts\)[\s\S]*resolveAndDownloadArtifact\(\{/u);
  assert.equal(JSON.stringify(registry).includes('https://huggingface.co/'), false);
});


test('typed bundle owns base/projector runtime roles even when profile projection order is reversed', () => {
  const reversedProfile = structuredClone(profile);
  reversedProfile.modelArtifacts.reverse();
  const reversedSelection = resolveActiveModelBundle({ registry: modelBundleRegistry, artifactRegistry, operationalProfile: reversedProfile });
  assert.equal(reversedSelection.state, 'MODEL_BUNDLE_RESOLVED');
  assert.equal(reversedSelection.artifacts[0].artifactRef, modelBundle.baseModelArtifactRef);
  assert.equal(reversedSelection.artifacts[1].artifactRef, modelBundle.projectorArtifactRef);
  const reversedPlan = buildVexInitializationPlan({ profile: reversedProfile, modelBundle: reversedSelection.bundle, modelArtifacts: reversedSelection.artifacts, home: 'C:/VexHome', homeState: 'FRESH_HOME_ALLOWED', hostEvidence: { platform: 'win32', architecture: 'x64' }, mode: 'candidate-qualification' });
  const plannedModels = reversedPlan.artifacts.filter((artifact) => artifact.destinationClass === 'MODEL');
  assert.equal(plannedModels[0].artifactRef, modelBundle.baseModelArtifactRef);
  assert.equal(plannedModels[1].artifactRef, modelBundle.projectorArtifactRef);
  const initializer = fs.readFileSync(path.join(ROOT, 'scripts/initialize-vex.mjs'), 'utf8');
  assert.equal(/profile\.modelArtifacts\[[01]\]/u.test(initializer), false);
  assert.match(initializer, /artifactPaths\.get\(modelBundle\.baseModelArtifactRef\)/u);
  assert.match(initializer, /artifactPaths\.get\(modelBundle\.projectorArtifactRef\)/u);
  assert.match(initializer, /activeArtifactRef: modelBundle\.baseModelArtifactRef/u);
});

test('model-bundle registry rejects a noncanonical registryRef', () => {
  const wrong = structuredClone(modelBundleRegistry);
  wrong.registryRef = 'registry.vexlife.model-bundles.other';
  const validation = validateModelBundleRegistry(wrong, { artifactRegistry, operationalProfileRegistry: registry });
  assert.equal(validation.ok, false);
  assert.match(validation.errors.join('; '), /registryRef is not canonical/u);
});
