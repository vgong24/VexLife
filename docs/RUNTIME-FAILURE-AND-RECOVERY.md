...
Every persisted claim transition embeds an edge-specific source-managed evidence contract. The canonical checkpoint is an immutable content-addressed object; current checkpoint state lives only in a separate replayable pointer ledger. Each of its six release objects embeds the exact transition receipt plus exact prior and transitioned leases. Claim replay consumes a compact content-addressed checkpoint-scoped slice of the exact prior scheduler state: phase, generation, queue, active worker, runtime/resource/clock, canonical checkpoint and pointer, the required lease bindings, terminal tail and exact prior claim aggregate/transition. It never nests an earlier state slice or duplicates a prior edge receipt. Registered byte budgets bound every slice and each aggregate increment linearly across sequential lifecycle transitions while restart still restores the exact state. A sibling paused slice or a coordinated checkpoint, release, claim, consumption, pointer or aggregate rehash cannot substitute for it. Pointer advancement never rewrites the canonical checkpoint.
...
Serialization persists the event ledger and derived snapshot. Restore replays the ledger and compares every persisted derived field with the replay result. Historical cycles remain immutable while active pointers move to the newest cycle. Old action, success, convergence or terminal evidence cannot satisfy a later cycle, including same-class/same-operation recurrence; a current success must follow the current action, scheduler continuation and fresh generation. Every generic, wait, resume and split external event carries the exact current scheduler claim lifecycle receipt ref and fingerprint. Managed wait/resume/split events require `CLAIMED_CURRENT`; generic events may also use `RESUMED_CONSUMED`. Invalidated and terminal claims reject every external event before mutation with exact typed reasons, and event-ledger replay revalidates the same lifecycle binding. Impossible order, budget reset, forged final state, duplicate terminal closure within one cycle, stale external events and same-ref/different-content events are rejected. Exact duplicates are semantic no-ops.
